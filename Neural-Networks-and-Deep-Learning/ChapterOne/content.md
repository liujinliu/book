...
我们将试着找到一种方式得到\\(\Delta v_1\\), \\(\Delta v_2\\), 使得\\(\Delta C\\)是负数. 这样就能保证我们的小球是向山谷下方运动的. 我们定义\\(\Delta v\\)表示\\(v\\)的变化, \\(\Delta v \equiv (\Delta v_1, \Delta v_2)^T\\), 我们同时定义\\(C\\)的梯度为\\((\frac{\partial C}{\partial v_1}, \frac{\partial C}{\partial v_2})^T\\).