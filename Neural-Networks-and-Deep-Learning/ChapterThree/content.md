...
softmax的想法来自于想给我们的神经网络定一个不同的输出层. 就像我们在S型神经元用到的. 定义加权输入为\\(z^L_j = \sum_k w^L_{jk} a^{L-1}_k + b^L_j\\). 但是我们得到输出所采用的公式略有不同, 我们在softmax层对\\(z_j^L\\)应用softmax函数, 我们得到第j个输出神经元的激励\\(a_j^L\\)为:  
